...
| --- | --- | --- | --- | --- |
| **チャット** | `chats`（uid_from / uid_to / body） | 投稿・取得・状態 | あり | ほぼ動く（宛先が `999` 固定、オンライン状態は未実装）→ **作り直して取り込み** |
...
参考元で唯一まともに動いていた機能。ただし**実態は宛先の無い単一のルーム**だった。

- `uid_to` は投稿時に `999` が固定で入るだけ
- 取得は全件から新しい 15 件を返すだけで、`uid_from` / `uid_to` を見ていない
- 画面の宛先一覧は「ユーザー１」が 3 つ並ぶ静的な HTML で、宛先を選ぶ手段が無い

使われていなかった `uid_to` は持たせず、**全員が読み書きする 1 つのルーム**として作り直した。`uid_from` は対が無くなったので `user_id` に改名している。

### 参考実装から変えたところ

| 参考実装 | このアプリ | 理由 |
| --- | --- | --- |
| 宛先 (`uid_to`) が 999 固定 | **列ごと持たない** | 一度も意味のある値が入らず、取得側も見ていなかった |
| 投稿者を `session('id')` で入れる | 同じ（**クライアントから受け取らない**） | 画面から `user_id` を送らせると他人になりすませる。テストで無視されることを確かめている |
| 検証なし（`$request->body` を直接保存） | FormRequest + ドメインの検証 | 空の発言がそのまま保存できていた |
| `body` が `string(255)` | `text` | 改行を含む発言が入る |
| 誰がいつ発言したか画面に出ない | 投稿者名と日時を出す | 参考実装は本文しか描かず、日時の処理はコメントアウトされていた |
| 削除の手段が無い | **自分の発言だけ削除できる** | 人の発言を消せると議論の記録が一方的に失われる |
| axios で `/chat/get` を 10 秒ごとに叩き、全件で DOM を作り直す | **Inertia の部分リロード**（`router.reload({ only: ['messages'] })`） | 専用の JSON エンドポイントを増やさずに済む。差分を見ずに `innerHTML` を空にして組み直す実装も不要になる |
| 新しい順のまま画面へ渡し JS で `reverse()` | サーバー側で古い順にして渡す | 並べ替えは画面の仕事ではない |
| 15 件固定 | 50 件（`ListChatMessagesUseCase::DEFAULT_LIMIT`） | — |
| オンライン状態（`return $request['status']` で何もしない） | **持たない** | 実装されていなかったうえ、ポーリングで状態を持つと複雑さが見合わない |

### チャットに残っている制約

- 1 ルームのみ。宛先や複数チャンネルは無い
- 更新は 10 秒ごとのポーリング。WebSocket は使っていない
- 既読・メンション・添付は無い

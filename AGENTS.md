...
- **画面は全 42 枚が Inertia + Svelte 5**。Blade として残っているのはルートテンプレート (`app.blade.php`)、エラーページ (`errors/`)、メール本文 (`emails/`) だけ
...
- **状態と提出日・講評は必ず噛み合う**。未提出は日付を持たず、**提出済みの段階では講評を持たない**（提出される前に評価はできない）
- **同じ提出者・同じ課題の提出物は 2 つ作らない**。再提出は既存の行を更新する
- **遅延は提出日と期限から導く**。カラムには持たない
### チャット

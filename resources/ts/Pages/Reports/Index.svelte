<script lang="ts">
  ...
  let confirm = $state<DeleteConfirm<Report> | undefined>();
</script>
...
<DeleteConfirm bind:this={confirm} title="勤務報告の削除">
  {#snippet body(row: Report)}{row.dateLabel}「{row.title}」を削除します。取り消せません。{/snippet}
</DeleteConfirm>

<script lang="ts">
  ...
  let confirm = $state<DeleteConfirm<JournalEntry> | undefined>();
</script>
...
<DeleteConfirm bind:this={confirm} title="仕訳の削除">
  {#snippet body(row: JournalEntry)}{row.dateLabel}「{row.description}」（￥{row.amountLabel}）を削除します。取り消せません。{/snippet}
</DeleteConfirm>
